...
# 定义一个方法，给线程使用
def download_ts(ts_i):
    u = url_prifix + ts_i + ".ts"
    r = requests.get(url=u, headers=headers).content
    print(ts_i, u)
    s = '%d' % int(ts_i)  # 整型转为字符串
    with open('./ts003/' + s + '.ts', mode='wb') as file:
        file.write(r)


for i in ts:
    executor.submit(download_ts, i)

# 解密
# openssl_cmd = "openssl aes-128-cbc -d -in ./ts002/" +s+ ".ts -out ./ts0022/0" +s+ ".ts -iv 00000000000000000000000000000000 -K  37613432633731363239396236663036"
# os.system(openssl_cmd)

# 合成
# ffmpeg_cmd = "ffmpeg -i file_list.txt -c:a copy -bsf:a aac_adtstoasc new.mp4"
# os.system(ffmpeg_cmd)

# 源 source_folder
# 目的 destination_folder
